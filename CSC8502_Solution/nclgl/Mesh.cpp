...
Mesh* Mesh::GenerateTriangle()
{
	...
	m->textureCoords[0] = Vector2(0.5, 0.0f);
...
/* 
	Repurposed terrain generation from the HeightMap class to generate a flat terrain
	Note: scale values hardcoded to the used terrain map image dimensions
	~ consider adding image width and height input in the future ~
*/
Mesh* Mesh::GenerateFlatTerrain(float height) {
	int const WIDTH = 256;
	int const HEIGHT = 256;
	Mesh* m = new Mesh();

	m->numVertices = WIDTH * HEIGHT;
	m->type = GL_TRIANGLES;

	m->numVertices = WIDTH * HEIGHT;
	m->numIndices = (WIDTH - 1) * (HEIGHT - 1) * 6;
	...
	for (int z = 0; z < HEIGHT; ++z) {
		for (int x = 0; x < WIDTH; ++x) {
			...
			m->vertices[offset] = Vector3(x, height, z) * vertexScale;

			m->textureCoords[offset] = Vector2(x, z) * textureScale;
		}
	}
...
Mesh* Mesh::GenerateRandomPositions(int count)
{
	Mesh* m = new Mesh();
	m->numVertices = count;
	m->type = GL_POINTS;
	...
	for (int i = 0; i < count; i++) {
		m->vertices[i] = Vector3(rand() % 8100, rand() % 1000 + 1200, rand() % 8100);
...
Mesh* Mesh::GenerateRandomLightningPositions(int count)
{
	Mesh* m = new Mesh();
	m->numVertices = count;
	m->type = GL_POINTS;
	...
	for (int i = 0; i < count; i++) {
		m->vertices[i] = Vector3(rand() % 8100, 1800, rand() % 8100);
		m->colours[i] = Vector4(0.0f, 0.0f, 1.0f, 1.0f);
	}
	m->BufferData();
	return m;

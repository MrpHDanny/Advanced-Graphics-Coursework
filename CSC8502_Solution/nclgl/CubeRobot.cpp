...
CubeRobot::CubeRobot(Mesh* cube)
{
	...
	rightArm = new SceneNode(cube, Vector4(0, 0, 1, 1)); // Green
	rightArm->SetModelScale(Vector3(3, -18, 3));
	rightArm->SetTransform(Matrix4::Translation(Vector3(12, 30, -1)));
	body->AddChild(rightArm);

	leftLeg = new SceneNode(cube, Vector4(0, 0, 1, 1)); // Green
	leftLeg->SetModelScale(Vector3(3, -17.5, 3));
	leftLeg->SetTransform(Matrix4::Translation(Vector3(-8, 0, 0)));
	body->AddChild(leftLeg);

	rightLeg = new SceneNode(cube, Vector4(0, 0, 1, 1)); // Green
...
void CubeRobot::Update(float dt)
{
	...
	rightArm->SetTransform(leftArm->GetTransform() * Matrix4::Rotation(30.0f * dt, Vector3(1, 0, 0)));
